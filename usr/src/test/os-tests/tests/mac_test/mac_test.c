...
build_frag_v6(mac_ether_offload_info_t *meoi)
{
	 ...
	 */
	assert(tp->tp_sz == sizeof (struct ether_header));
	tp_append(tp, &hdr_ip6, sizeof (hdr_ip6));
...
run_test(nvlist_t *payload, char *test_tuple)
{
	...
	assert(mktemp(payload_path) != NULL);

	size_t payload_sz;
	char *payload_packed = fnvlist_pack(payload, &payload_sz);
	assert(payload_packed != NULL);
	nvlist_free(payload);

	int fd = open(payload_path, O_CREAT | O_TRUNC | O_WRONLY, 0);
	if (fd < 0) {
		err(EXIT_FAILURE, "failed to open payload file");
	}
	ssize_t written = write(fd, payload_packed, payload_sz);
	if (written != payload_sz) {
		err(EXIT_FAILURE, "unable to write full payload, %u != %d",
		    payload_sz, written);
	}
...
main(int argc, char *argv[])
{
	...
	test_pkt_t *tp_tcp4 = build_tcp4();
	mac_ether_offload_info_t meoi_tcp4 = {
		.meoi_flags =
		    MEOI_L2INFO_SET | MEOI_L3INFO_SET | MEOI_L4INFO_SET,
		.meoi_len = tp_tcp4->tp_sz,
		.meoi_l2hlen = sizeof (struct ether_header),
		.meoi_l3proto = ETHERTYPE_IP,
		.meoi_l3hlen = sizeof (struct ip),
		.meoi_l4proto = IPPROTO_TCP,
	...
	test_pkt_t *tp_tcp6 = build_tcp6();
	mac_ether_offload_info_t meoi_tcp6 = {
		.meoi_flags =
		    MEOI_L2INFO_SET | MEOI_L3INFO_SET | MEOI_L4INFO_SET,
		.meoi_len = tp_tcp6->tp_sz,
		.meoi_l2hlen = sizeof (struct ether_header),
		.meoi_l3proto = ETHERTYPE_IPV6,
	...
	any_failed |= !run_meoi_variants("basic tcp4", tp_tcp4, &meoi_tcp4);
	any_failed |= !run_meoi_variants("basic tcp6", tp_tcp6, &meoi_tcp6);
	...
	any_failed |=
	    !run_meoi_variants("fragment ipv4", tp_frag_v4, &meoi_frag_v4);
	any_failed |=
	    !run_meoi_variants("fragment ipv6", tp_frag_v6, &meoi_frag_v6);
