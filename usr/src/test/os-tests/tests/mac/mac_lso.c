...
main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "4fpeb:s:m:")) != -1) {
		...
		case 'm':
			errno = 0;
			popts.po_mss = strtoul(optarg, NULL, 0);
			if (errno != 0 || popts.po_mss == 0) {
				err(EXIT_FAILURE,
				    "invalid MSS value %s", optarg);
			}
			break;
	...
	if (popts.po_cksum_full == popts.po_cksum_partial) {
		err(EXIT_FAILURE, "specify exactly one of -f/-p");
	}

	int in_fd = open(argv[0], O_RDONLY, 0);
	if (in_fd < 0) {
		err(EXIT_FAILURE, "could not open input cap file %s", argv[0]);
	}

	int out_fd = open(argv[1], O_RDONLY, 0);
	if (out_fd < 0) {
