...
/*
 * Frees a vmm_gpt_node_t structure and its corresponding page
 * of NPT entries.
 */
static void
vmm_gpt_node_free(vmm_gpt_node_t *node)
{
	...
	ASSERT3P(node->vgn_children, ==, NULL);
	ASSERT3P(node->vgn_siblings, ==, NULL);
	ASSERT3P(node->vgn_parent, ==, NULL);

	kmem_free((void *)node->vgn_entries, PAGESIZE);
 ...
 */
static void
vmm_gpt_node_free_tree_node(vmm_gpt_node_t *node)
{
	ASSERT(node != NULL);
	ASSERT3U(node->vgn_ref_cnt, ==, 0);
	node->vgn_children = NULL;
	...
	node->vgn_siblings = NULL;
	node->vgn_parent = NULL;
...
vmm_gpt_node_tree_free(vmm_gpt_node_t *node)
{
	...
	if (node->vgn_children != NULL)
		vmm_gpt_node_tree_free(node->vgn_children);
	for (vmm_gpt_node_t *sibling = node->vgn_siblings, *next = NULL;
	    sibling != NULL;
	    sibling = next) {
		next = sibling->vgn_siblings;
		if (sibling->vgn_children != NULL)
			vmm_gpt_node_tree_free(sibling->vgn_children);
		vmm_gpt_node_free_tree_node(sibling);
	}
	vmm_gpt_node_free_tree_node(node);
}
...
/*
 * Removes and frees a child node from the a parent node in the GPT.
 */
static void
...
/*
 * Remove a mapping from the table.  Returns 0 if the page was not
 * mapped, otherwise returns nonzero.
 */
bool
