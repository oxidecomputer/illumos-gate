 ...
 * separate lock from the hash tables, which i_dls_devnet_hashmap_write will
 * ensure is held.
 *
 * i_dls_devnet_mut_lock protects the hash tables themselves. Taking a write on
 * it requires we first hold i_dls_devnet_mut_lock. Thus, we can safely drop,
 * reacquire, and upgrade/downgrade it so long as all table updates occur in a
 * single write.
 */
static kmutex_t		i_dls_devnet_mut_lock;
static kcondvar_t	i_dls_devnet_mut_cv;
static uintptr_t	i_dls_devnet_mut_own;
...
static void
i_dls_devnet_mut_lock_enter()
{
	mutex_enter(&i_dls_devnet_mut_lock);
	while (i_dls_devnet_mut_own != 0) {
		cv_wait(&i_dls_devnet_mut_cv, &i_dls_devnet_mut_lock);
	}
}

static void
i_dls_devnet_mut_lock_exit()
{
	VERIFY3U(i_dls_devnet_mut_own, ==, 0);
	cv_broadcast(&i_dls_devnet_mut_cv);
	mutex_exit(&i_dls_devnet_mut_lock);
}

static void
i_dls_devnet_mut_lock_upcall_start()
{
	VERIFY(MUTEX_HELD(&i_dls_devnet_mut_lock));
	VERIFY3U(i_dls_devnet_mut_own, ==, 0);
	i_dls_devnet_mut_own = (uintptr_t)curthread;
	mutex_exit(&i_dls_devnet_mut_lock);
}

static void
i_dls_devnet_mut_lock_upcall_end()
{
	mutex_enter(&i_dls_devnet_mut_lock);
	VERIFY3U(i_dls_devnet_mut_own, ==, (uintptr_t)curthread);
	i_dls_devnet_mut_own = 0;
}
...
dls_mgmt_init(void)
{
	...
	mutex_init(&i_dls_devnet_mut_lock, NULL, MUTEX_DEFAULT, NULL);
	cv_init(&i_dls_devnet_mut_cv, NULL, CV_DEFAULT, NULL);
	i_dls_devnet_mut_own = 0;
...
dls_mgmt_fini(void)
{
	...
	cv_destroy(&i_dls_devnet_mut_cv);
	mutex_destroy(&i_dls_devnet_mut_lock);
