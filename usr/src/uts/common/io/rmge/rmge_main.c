...
static void
rmge_dismantle(rmge_t *rmge)
{
	VERIFY3P(rmge, !=, NULL);
	VERIFY3P(rmge->dip, !=, NULL);
	VERIFY3P(ddi_get_driver_private(rmge->dip), ==, rmge);
	VERIFY(rmge->att_milestone & RMGE_ATT_MILESTONE_SOFTSTATE);
	VERIFY3P(rmge->mh, ==, NULL);
...
rmge_register_mac_device(rmge_t *rmge)
{
	...
	mr->m_dip = rmge->dip;
	mr->m_instance = 0;
	mr->m_src_addr = rmge->hw.mac_addr;
...
rmge_attach(dev_info_t *devinfo, ddi_attach_cmd_t cmd)
{
	...
	if (rmge_map_csrs(rmge) != RMGE_SUCCESS) {
		cmn_err(CE_WARN, "failed to map csrs for rmge");
		goto rollback;
	}

	if (rmge_identify_device_particulars(rmge) != RMGE_SUCCESS) {
		cmn_err(CE_WARN, "failed to identify hw rev");
		goto rollback;
	}

	if (rmge_drive_to_reset(rmge) != RMGE_SUCCESS) {
		cmn_err(CE_WARN, "failed to reset hw device");
		goto rollback;
	}
...
rmge_detach(dev_info_t *devinfo, ddi_detach_cmd_t cmd)
{
	...
	VERIFY3P(rmge->dip, ==, devinfo);
	VERIFY3S(rmge->instance, ==, ddi_get_instance(devinfo));
	VERIFY(rmge->att_milestone & RMGE_ATT_MILESTONE_SOFTSTATE);
