    ...
    int sgl_only)
{
	...
	if (txinfo->flags != 0) {
		 ...
		 */
		mac_ether_fullpktinfo(m, &txinfo->outer_info,
		    &txinfo->inner_info);

		if (txinfo->outer_info.meoi_tuntype != METT_NONE &&
		    txinfo->outer_info.meoi_flags == MEOI_FULLTUN)
    ...
    uint64_t *ctrlp)
{
	...
	const boolean_t needs_l3_csum = ((tx_flags & HW_LSO) != 0 || (tx_flags &
	    l3_cso_flag) != 0) && ulp_info->meoi_l3proto == ETHERTYPE_IP;
...
/*
 * For tunnelled CSO/LSO, we need to fill the outer IPv4 checksum if applicable.
 * - in CSO, this is the standard IPv4 checksum.
 * - in LSO we also invert, then remove the IP Total Length.
 */
static void
tun_fill_partial_v4(mblk_t **mp, const boolean_t is_lso,
    const mac_ether_offload_info_t *tuninfo)
{
	ipha_t *iph = NULL;
	size_t ip_off, ip_max;

	if (tuninfo->meoi_flags != MEOI_FULLTUN ||
	    tuninfo->meoi_tuntype == METT_NONE ||
	    tuninfo->meoi_l3proto != ETHERTYPE_IP)
		return;

	ip_off = tuninfo->meoi_l2hlen;
	ip_max = ip_off + tuninfo->meoi_l3hlen;

	if (MBLKL(*mp) < ip_max) {
		mblk_t *new = msgpullup(*mp, ip_off);
	...
	/*
	 * This is something of a workaround pending finer-grained flags.
	 * Tunnel devices should be populating checksums themselves.
	 */
	iph = (ipha_t *)((*mp)->b_rptr + ip_off);
	if (iph->ipha_hdr_checksum == 0)
    ...
    struct txinfo *txinfo)
{
	...
	if (is_tunneled && txinfo->flags != 0)
		tun_fill_partial_v4(&m, do_tso, outer_info);
