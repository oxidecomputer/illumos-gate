...
xhci_hcdi_pipe_close(usba_pipe_handle_data_t *ph, usb_flags_t usb_flags)
{
	 ...
	 * endpoints configured (see endpoint management comments in xhci.c).
	 * Periodic endpoints maintain a bandwidth reservation which inhibits
	 * the use of other devices, so we attempt to unconfigure those here.
	 */
