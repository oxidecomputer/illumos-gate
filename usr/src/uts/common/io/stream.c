...
dblk_constructor(void *buf, void *cdrarg, int kmflags)
{
	...
	bzero(dbp->db_mett, sizeof (dbp->db_mett));
...
{
	/*
	 * The first word of each of these fields contains the mexx_flags
	 * field in mac_ether_offload_info_t and mac_ether_tun_info_t, marking
	 * existing values as garbage.
	 */
	dbp->db_meoi[0] = 0;
	dbp->db_mett[0] = 0;
}
