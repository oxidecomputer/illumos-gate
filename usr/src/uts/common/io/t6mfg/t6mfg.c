...
static int t6mfg_srom_read(t6mfg_devstate_t *devstate_p, struct uio *uio_p, cred_t *cred_p) {
	while (uio_p->uio_offset <= T6MFG_SROM_MAX_ADDRESS && uio_p->uio_resid > 0) {
		...
		dev_err(devstate_p->dip, CE_WARN, "%s: read SROM address: %04x VPD address: %04x data: %08x",
				__FUNCTION__, vpd_dword_address + T6MFG_VPD_TO_SROM_OFFSET, vpd_dword_address, vpd_dword_data);
...
static int t6mfg_srom_write(t6mfg_devstate_t *devstate_p, struct uio *uio_p, cred_t *cred_p) {
	while (uio_p->uio_offset <= T6MFG_SROM_MAX_ADDRESS && uio_p->uio_resid > 0) {
		...
		dev_err(devstate_p->dip, CE_WARN,
			"%s: write SROM address %04llx, VPD address %04x, VPD dword address: %04x,"
		        " vpd_dword_byte_offset: %04x uio_resid: %ld",
			__FUNCTION__, uio_p->uio_offset, vpd_address, vpd_dword_address,
			vpd_dword_byte_offset, uio_p->uio_resid);
		...
		if (vpd_dword_byte_offset != 0 || uio_p->uio_resid < sizeof(uint32_t)) {
			...
			dev_err(devstate_p->dip, CE_WARN, "%s: unaligned write loaded existing dword %08x",
					__FUNCTION__, vpd_dword_data);
		}
		...
		dev_err(devstate_p->dip, CE_WARN, "%s: write VPD address: %04x data: %08x",
				__FUNCTION__, vpd_dword_address, vpd_dword_data);
...
static int t6mfg_spidev_ioctl(t6mfg_devstate_t *devstate_p, int cmd, intptr_t arg, int mode, cred_t *cred_p, int *rval_p) {
	...
	for (int xfer_idx = 0; xfer_idx < nxfers; xfer_idx++) {
		...
		int deassert_cs_after_xfer = ((xfer_idx + 1) == nxfers) || STRUCT_FGET(xfer, deassert_cs_after_xfer);
