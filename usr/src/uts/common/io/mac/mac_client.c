...
mac_tx_needed_offloads(const mac_impl_t *mip, mblk_t **mpp)
{
	...
	/*
	 * All cases from this point are tunnelled. If no offloads are
	 ...
	 *
	 * First, check whether this information has already been set by a
	 * client. If so, we can avoid attempting a parse.
	 *
	 * `fullpktinfo` extraction can fail (err != 0) on an otherwise
	 * well-formed packet -- ARP and ICMP carried as inner payloads, for
	 * example. But we want those to proceed here as we may have outer
	 * offloads or inner v4 checksums that need to be processed.
	 */
	mac_unpack_pktinfo(mp, &inner);
	if ((inner.meoi_flags & MEOI_L3INFO_SET) == 0) {
		(void) mac_ether_offload_info(mp, NULL, &inner);

		/* Setting inner L2 info requires a successful tunnel parse. */
