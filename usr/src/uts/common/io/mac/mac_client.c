...
mac_action_set(mac_client_handle_t mch, const flow_action_t *action)
{
	...
	/*
	 * The MAC client *must* have a defined action. In future, MAC clients
	 * should be part of a larger flow tree asociated with the provider --
	 * when this is the case we can accept 'delegate' actions here.
	 * However, as we cannot currently delegate here we need a definite
	 * action.
	 */
...
mac_action_clear(mac_client_handle_t mch)
{
	...
	VERIFY0(mac_action_set(mch, &default_ac));
}
