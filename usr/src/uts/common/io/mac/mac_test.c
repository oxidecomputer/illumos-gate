...
{
	mblk_t *mp = NULL;
	mac_ether_tun_info_t *tuninfo = NULL;
    ...
    enum mt_cksum_state csum_check)
{
	...
	for (mblk_t *curr = *mp; curr != NULL; curr = curr->b_next, ++i) {
		...
		mac_ether_tun_info_t *tuninfo = DB_METT(curr);
		...
		DTRACE_PROBE4(mac__test__verpkt, mblk_t *, curr,
		    size_t, i, mac_ether_tun_info_t *, tuninfo,
		    mac_ether_offload_info_t *, &meoi);
		...
		tuninfo->mett_flags = MEOI_TUNINFO_SET;
		tuninfo->mett_tuntype = tuntype;
		err = mac_ether_tun_info(curr, &encap_len);
		KT_ASSERT3SG(err, ==, 0, ctx, cleanup);
		err = mac_ether_offload_info_from(curr, &meoi, encap_len);
		KT_ASSERT3SG(err, ==, 0, ctx, cleanup);

		switch (tuninfo->mett_l3proto) {
		case ETHERTYPE_IP:
			oip4 = (ipha_t *)(curr->b_rptr + tuninfo->mett_l2hlen);
			...
			break;
		case ETHERTYPE_IPV6:
			oip6 = (ip6_t *)(curr->b_rptr + tuninfo->mett_l2hlen);
		...
		ol4 = (udpha_t *)(curr->b_rptr + tuninfo->mett_l2hlen +
		    tuninfo->mett_l3hlen);
		iip = (ipha_t *)(curr->b_rptr + encap_len +
		    meoi.meoi_l2hlen);
		il4 = (tcpha_t *)((void *)iip + meoi.meoi_l3hlen);
		...
		if (csum_check & MT_CSUM_OUTER) {
			...
			uint32_t sum = 0;
			if (tuninfo->mett_l3proto == ETHERTYPE_IP)
				sum = mt_pseudo_sum(IPPROTO_UDP, oip4);
			else if (tuninfo->mett_l3proto == ETHERTYPE_IPV6)
...
mac_sw_cksum_tun_ipv4_test(ktest_ctx_hdl_t *ctx)
{
	...
	mp = mt_generate_tunpkt(ETHERTYPE_IP, METT_GENEVE, 0, ETHERTYPE_IP,
	    IPPROTO_TCP, bodylen, mss, HCK_IPV4_HDRCKSUM | HCK_FULLCKSUM);
	KT_EASSERT3P(mp, !=, NULL, ctx);
...
mac_tun_info_test(ktest_ctx_hdl_t *ctx)
{
	...
	KT_EASSERT3P(mp, !=, NULL, ctx);
	err = mac_ether_tun_info(mp, &encap_len);
	KT_ASSERT3SG(err, ==, 0, ctx, cleanup);

	tuninfo = DB_METT(mp);
	KT_ASSERT3UG(tuninfo->mett_flags, ==, MEOI_FULLTUN, ctx, cleanup);
	KT_ASSERT3UG(tuninfo->mett_l2hlen, ==, sizeof (struct ether_header),
	    ctx, cleanup);
	KT_ASSERT3UG(tuninfo->mett_l3proto, ==, ETHERTYPE_IP, ctx, cleanup);
	KT_ASSERT3UG(tuninfo->mett_l3hlen, ==, sizeof (ipha_t), ctx, cleanup);
