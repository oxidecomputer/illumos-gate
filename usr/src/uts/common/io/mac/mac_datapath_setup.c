    ...
    mac_srs_fanout_state_t state)
{
	 ...
	 */
	rw_enter(&mac_srs->srs_logical_lock, RW_READER);
	...
	rw_exit(&mac_srs->srs_logical_lock);
}
...
mac_srs_update_drain_proc(mac_soft_ring_set_t *srs)
{
	...
	if (is_forward) {
		...
		if (has_subflows) {
			if (subflows_are_bw) {
				drain_fn = mac_rx_srs_drain_bw_subtree_bw;
			} else {
				drain_fn = mac_rx_srs_drain_bw_subtree;
			}
		} else {
		...
		if (has_subflows) {
			if (subflows_are_bw) {
				drain_fn = mac_rx_srs_drain_subtree_bw;
			} else {
				drain_fn = mac_rx_srs_drain_subtree;
			}
		} else {
    ...
    const flow_entry_t *flent, const mac_bw_ctl_t *bw, const bool enable)
{
	...
	rw_enter(&srs->srs_logical_lock, RW_READER);
...
mac_srs_destroy_flowtree(mac_soft_ring_set_t *srs)
{
	...
	VERIFY(rw_write_held(&srs->srs_logical_lock));
    ...
    const mac_soft_ring_set_type_t debug_flags)
{
	VERIFY3P(srs, !=, NULL);
	mutex_enter(&srs->srs_lock);
	rw_enter(&srs->srs_logical_lock, RW_WRITER);
	...
	for (uint16_t i = 0; i < rx_srs_cnt; i++) {
		...
		case SRS_FANOUT_UNINIT:
			mac_srs_fanout_init(mcip, mrp, mac_rx_srs, mac_tx_srs,
			    cpupart);
			rw_enter(&mac_rx_srs->srs_logical_lock, RW_WRITER);
			VERIFY0(mac_flow_baked_tree_create(
			    mcip->mci_rx_flow_tree, mac_rx_srs));
			rw_exit(&mac_rx_srs->srs_logical_lock);
			if (mac_tx_srs != NULL) {
				rw_enter(&mac_tx_srs->srs_logical_lock,
				    RW_WRITER);
				VERIFY0(mac_flow_baked_tree_create(
				    mcip->mci_tx_flow_tree, mac_tx_srs));
				rw_exit(&mac_tx_srs->srs_logical_lock);
			}
			break;
		...
		case SRS_FANOUT_REINIT:
			 ...
			 *
			 * Note that we deliberately take srs_logical_lock
			 * _after_ modifying the fanout of the complete SRS.
			 * Per-fanout-lane kstats live on the softrings, and
			 * require a readlock on the logicals when requested.
			 * This ordering ensures kstat destruction can proceed.
			 */
			rw_enter(&mac_rx_srs->srs_logical_lock, RW_WRITER);
    ...
    const struct mac_srs_create_params *p)
{
	...
	mutex_init(&mac_srs->srs_lock, NULL, MUTEX_DEFAULT, NULL);
	rw_init(&mac_srs->srs_logical_lock, NULL, RW_DEFAULT, NULL);
	...
	if (is_logical) {
		...
		VERIFY(rw_write_held(&parent->srs_logical_lock));
	...
	rw_enter(&flent->fe_srs_list_lock, RW_WRITER);
	list_insert_tail(&flent->fe_srs_list, mac_srs);
...
mac_srs_free(mac_soft_ring_set_t *mac_srs)
{
	...
	if (is_complete_srs) {
		mutex_enter(&mac_srs->srs_lock);
		rw_enter(&mac_srs->srs_logical_lock, RW_WRITER);
	...
	/*
	 * Fold any acquired stats into the flent, and remove ourself from
	 * its overall list.
	 */
	flow_entry_t *flent = mac_srs->srs_flent;
	rw_enter(&flent->fe_srs_list_lock, RW_WRITER);
	list_remove(&flent->fe_srs_list, mac_srs);
	if (is_tx) {
		atomic_add_64(&flent->fe_match_pkts_out,
		    mac_srs->srs_match_pkts);
		atomic_add_64(&flent->fe_match_bytes_out,
		    mac_srs->srs_match_bytes);
	} else {
		atomic_add_64(&flent->fe_match_pkts_in,
		    mac_srs->srs_match_pkts);
		atomic_add_64(&flent->fe_match_bytes_in,
		    mac_srs->srs_match_bytes);
	}
	rw_exit(&flent->fe_srs_list_lock);
	...
	rw_destroy(&mac_srs->srs_logical_lock);
	mutex_destroy(&mac_srs->srs_lock);
    ...
    const mac_soft_ring_set_state_t logical_flag)
{
	...
	VERIFY(!mac_srs_is_logical(mac_srs));
	VERIFY(rw_lock_held(&mac_srs->srs_logical_lock));
...
mac_flow_tree_create_walker(void *arg, const mac_flow_tree_walker_ctx_t *ctx)
{
	...
	if (ctx->mftw_is_enter) {
		...
		VERIFY(effective_ty == MFA_TYPE_DROP ||
		    curr_delegate->srs != NULL);
