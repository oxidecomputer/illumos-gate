	...
	while (logical_srs != NULL) {
		...
		for (mac_soft_ring_t *softring =
			...
			if (add_notify_fn != NULL &&
				...
				mrf.mrf_flow_priority = logical_srs->srs_pri;

				softring->s_ring_rx_arg2 = add_notify_fn(
				    act->fa_resource_arg,
				    (mac_resource_t *)&mrf);
			}
		}
    ...
    mac_soft_ring_set_t *mac_srs)
{
	...
	while (logical_srs != NULL) {
		...
		for (mac_soft_ring_t *softring =
			 ...
			 * to redefine an action today, so no need for this
			 * normally.
			 */
			if (rm_notify_fn != NULL &&
			    softring->s_ring_rx_arg2 != NULL) {
				rm_notify_fn(act->fa_resource_arg,
				    softring->s_ring_rx_arg2);
			}
			...
			softring->s_ring_state &= ~S_RING_CLIENT_WAIT;
			softring->s_ring_rx_arg2 = NULL;
			/* TODO(ky): take from the actual flent assoc'd? */
			softring->s_ring_rx_func = mac_rx_deliver;
			softring->s_ring_rx_arg1 = (void *)mcip;
    ...
    cpupart_t *cpupart)
{
	...
	mac_srs_tx_t *srs_tx = &tx_srs->srs_kind_data.tx;
...
mac_walk_srs_and_bind(int cpuid)
{
	...
	for (; mac_srs != NULL; mac_srs = mac_srs->srs_next) {
		...
		if (!mac_srs_is_tx(mac_srs)) {
			if (mac_srs->srs_kind_data.rx.sr_poll_cpuid == -1 &&
			    mac_srs->srs_kind_data.rx.sr_poll_cpuid_save == cpuid) {
				mac_srs->srs_kind_data.rx.sr_poll_cpuid_save = -1;
...
mac_update_srs_priority(mac_soft_ring_set_t *mac_srs, pri_t prival)
{
	...
	if (!mac_srs_is_tx(mac_srs) && mac_srs->srs_kind_data.rx.sr_poll_thr != NULL) {
		thread_lock(mac_srs->srs_kind_data.rx.sr_poll_thr);
		(void) thread_change_pri(mac_srs->srs_kind_data.rx.sr_poll_thr,
		    mac_srs->srs_pri, 0);
		thread_unlock(mac_srs->srs_kind_data.rx.sr_poll_thr);
	}
...
mac_srs_update_fanout_list(mac_soft_ring_set_t *mac_srs)
{
	...
	ASSERT3S(mac_srs->srs_soft_ring_count, ==, count);
}
...
void
mac_srs_create_rx_softring(int id, uint32_t type, pri_t pri,
    ...
    mac_soft_ring_set_t *mac_rx_srs, mac_soft_ring_set_t *mac_tx_srs)
{
	...
	const int new_fanout_cnt = srs_cpu->mc_rx_fanout_cnt;
	...
	const mac_resource_bind_t bind_notify_fn = act->fa_resource_bind;
	...
	ASSERT(new_fanout_cnt == mac_rx_srs->srs_soft_ring_count);
	mutex_enter(&cpu_lock);
...
no_softrings:
	if (mac_rx_srs->srs_type & SRST_FANOUT_PROTO) {
		...
		cpuid = mac_next_bind_cpu(cpupart);
		/* Create the protocol softrings */
		mac_srs_create_rx_softring(0, soft_ring_flag,
		    mac_rx_srs->srs_pri, mcip, mac_rx_srs, cpuid);
		mutex_exit(&cpu_lock);
	} else {
		/*
...
{
	const bool is_tx_srs = p->msc_ty == SCT_TX;
    ...
    uint32_t link_type)
{
	...
	if (flent->fe_rx_srs[0] == NULL) {
		...
		mac_srs = mac_srs_create_rx(mcip, flent,
		    fanout_type | link_type, mac_rx_deliver, NULL);
		mutex_enter(&flent->fe_lock);
		flent->fe_cb_fn =
		    (flow_fn_t)mac_srs->srs_kind_data.rx.sr_lower_proc;
		flent->fe_cb_arg1 = (void *)mip;
	...
	case MAC_GROUP_STATE_RESERVED:
		for (ring = rx_group->mrg_rings; ring != NULL;
			...
			case MR_FREE:
				 ...
				 */
				mac_srs = mac_srs_create_rx(mcip, flent,
				    fanout_type | link_type, mac_rx_deliver,
				    ring);
				break;
			default:
				cmn_err(CE_PANIC,
    ...
    uint32_t link_type)
{
	...
	mac_srs_tx_t *tx = &tx_srs->srs_kind_data.tx;
	switch (link_type) {
	case SRST_FLOW:
		/*
...
mac_walk_srs_and_unbind(int cpuid)
{
	...
	for (; mac_srs != NULL; mac_srs = mac_srs->srs_next) {
		...
		if (!mac_srs_is_tx(mac_srs)) {
			if (mac_srs->srs_kind_data.rx.sr_poll_cpuid == cpuid) {
				mac_srs->srs_kind_data.rx.sr_poll_cpuid_save = cpuid;
				mac_srs_poll_unbind(mac_srs);
			}
		}
...
mac_tx_srs_del_ring(mac_soft_ring_set_t *mac_srs, mac_ring_t *tx_ring)
{
	...
	mutex_exit(&mac_srs->srs_lock);
	ASSERT3S(i, <, mac_srs->srs_soft_ring_count);
