...
mac_flow_user_cpu_init(flow_entry_t *flent, mac_resource_props_t *mrp)
{
	...
	if (no_of_cpus >= (reqd_rx_cpu_cnt + reqd_tx_cpu_cnt)) {
		...
		VERIFY3P(rx_srs->srs_data.rx.sr_ring, ==, NULL);
		...
		if (flent->fe_rx_srs_cnt > 1) {
			...
			for (srs_cnt = 1;
				...
				VERIFY3P(rx_srs->srs_data.rx.sr_ring, !=,
				    NULL);
...
mac_srs_update_lower_proc(mac_soft_ring_set_t *srs)
{
	...
	if (srs->srs_data.rx.sr_lower_proc == MRSLP_HWRINGS) {
		return;
	}
 ...
 */
static size_t
mac_srs_num_active_bw(const mac_soft_ring_set_t *srs)
{
	size_t active = 0;
	mac_srs_bw_lock(srs);
...
mac_tx_srs_update_bwlimit_state(mac_soft_ring_set_t *srs)
{
	...
	const bool is_enabled = mac_srs_num_active_bw(srs) != 0;
...
mac_client_srs_vanity_flags(const mac_client_impl_t *mcip)
{
...
mac_client_rebuild_flowtrees(mac_client_impl_t *mcip, const bool do_tx)
{
	...
	const mac_soft_ring_set_type_t vanity_flags =
	    mac_client_srs_vanity_flags(mcip);
 ...
 *
 * Flows:
 *	Can be configured on Links. Flows on a link are combined
 *	together into a tree structure, allowing us to combine
 *	action specifiers, bandwidth controls. and CPU/priority
 *	allocations together when any combination of flows applies to
    ...
    const mac_soft_ring_set_type_t link_type)
{
	...
	grp_found:
		...
		if (vid != VLAN_ID_NONE) {
			...
			flent->fe_ft_match.arg.mfm_list =
			...
			bcopy(&mcip->mci_unicast->ma_addr,
			    flent->fe_ft_match.arg.mfm_l2addr, ETHERADDRL);
		}
 ...
 * or poll thread. This function is MAC-internal and is called from the higher
 * level quiesce/condemn/restrat functions on the SRS.
 */
void
...
/*
 * Create a logical SRS for each node of a flowtree. This function is called
 * for *every* node of a given flowtree, although we do not plan to use each one
 * in the ideal case.
 *
 * The handling of 'delegate' nodes is why this the case. When no bandwidth
 * limits are imposed on the subtree, we always delegate packets back to the
 * 'canonical' flow where possible. This ensures that we can pass over all
 * affected packets in one shot. E.g., when plumbing DLS bypass we have
 * intermediate nodes for (unfragmented) v4 and v6 -- we prefer that any non
 * TCP/UDP traffic is queued and processed on the root SRS all at once. The
 * exception is when a child flow's priority or fanout differs from that of its
 * parent, and we need to deliver to the logical SRS to meet those constraints.
 * So, in a world without bandwidth, we could build the tree with *only* SRSes
 * corresponding to `DO` or `DROP`.
 *
 ...
 * The other issue is that the flow tree exists outside of the `flow_entry_t`s
 ...
 *
 *  - f1: Limit IPv4 on CIDR 192.168.0.0/16 @ rate 1.
 *
...
mac_flow_tree_new_srs(flow_entry_t *ent, const bool is_tx, const bool quiesce,
    ...
    const size_t n_bw_members, const mac_cpus_t *fanout_blueprint,
    mac_soft_ring_set_t *root_srs, const flow_tree_t *curr_tree_node,
    const flow_tree_t *root_tree_node)
{
	VERIFY3P(curr_tree_node, !=, root_tree_node);
...
mac_flow_baked_tree_create(const flow_tree_t *ft, mac_soft_ring_set_t *based_on)
{
	...
	into->ftb_bw_count = (uint16_t)bw_set_count;
	if (n_nodes == 0) {
		VERIFY3U(max_depth, ==, 0);
	...
	while (el != NULL && el != ft) {
		VERIFY3U(depth, >, 0);
		VERIFY3U(curr_node - into->ftb_subtree, <=, 2 * UINT16_MAX);
		...
		if (!ascended) {
			...
			*srs_slot = (effective_ty == MFA_TYPE_DROP) ? NULL :
			...
			mac_flow_specialise_match(&curr_node->enter.ften_match,
			    is_tx);
