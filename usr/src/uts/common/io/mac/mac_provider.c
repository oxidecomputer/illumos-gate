...
/*
 * This file implements public methods for storing/manipulating packet info
 * within `dblk_t`s. This #define exposes the underlying representation.
 *
 * All actual consumers (apart from streams.c, and ktest methods) should go via
 * methods exported from mac_provider.h/mac_impl.h.
 */
#define _STREAM_DBLK_INTERNALS
...
/*
 * Attempt to read uint16_t (in network order) at offset `pos` in mblk chain.
 *
 ...
 * size will be populated on success.  The VLAN TCI will contain UINT32_MAX if
 * the header does not bear VLAN infomation.
 */
static bool
mac_mmc_parse_ether(mac_mblk_cursor_t *cursor, uint8_t *dst_addrp,
    uint32_t *vlan_tcip, uint32_t *l3_sapp, uint16_t *hdr_sizep)
{
 ...
 */
static bool
mac_mmc_parse_l3(mac_mblk_cursor_t *cursor, uint32_t l3_sap, uint8_t *ipprotop,
 ...
 */
static bool
mac_mmc_parse_l4(mac_mblk_cursor_t *cursor, uint16_t ipproto,
    uint8_t *hdr_sizep)
{
 ...
 *
 * If packet ethertype does not indicate that a VLAN is present, UINT32_MAX will
 * be returned for the TCI.
 */
int
 ...
 * information by setting MEOI_L2INFO_SET in `meoi_flags`, the L3 SAP in
 * `meoi_l3_proto`. With value in `meoi_l2hlen` that, when combined with the
...
/*
 * Perform a partial parsing of the encapsulation of a tunnelled ethernet
 ...
 * tunnelled/untunnelled packet.
 */
boolean_t
mac_meoi_is_full(const mac_ether_offload_info_t *mp)
{
	return ((mp->meoi_flags & MEOI_FULL) == MEOI_FULL);
}
 ...
 */
boolean_t
mac_tun_meoi_is_full(const mac_ether_offload_info_t *mp)
{
	return ((mp->meoi_flags & MEOI_FULLTUN) == MEOI_FULLTUN);
}
...
/*
 * Fill a `mac_ether_offload_info_t` using all flags and facts set on a packet
 * by a client, without performing any parsing.
 */
void
mac_unpack_pktinfo(const mblk_t *mp, mac_ether_offload_info_t *info)
{
 ...
 *
 * The function will return an error if the above constraints are violated, or
 * the underlying `dblk_t` has more than one reference in `db_ref`.
 */
int
    ...
    const mac_ether_offload_info_t *inner_info)
{
	...
	if (outer_info->meoi_tuntype == METT_NONE) {
		...
		err = pack_pktinfo(db, outer_info);
	} else {
		db->db_pktinfo.p_flags = 0;
		err = pack_tunpktinfo(db, outer_info);
	}
 ...
 * Returns an error (-1) if the packet has more than one reference to
 * its `dblk_t`.
 */
int
