...
mac_features_to_flags(mac_handle_t mh)
{
	...
	mac_capab_cso_t *cap_cso = &mip->mi_tx_cksum_capab;
	uint16_t flags = 0;
	 ...
	 */
	if (mac_capab_get(mh, MAC_CAPAB_LSO, &mip->mi_tx_lso_capab))
		flags |= HW_LSO;
...
static inline void
unpack_pktinfo_inline(const dblk_t *db, mac_ether_offload_info_t *info)
{
...
static inline void
unpack_tunpktinfo_inline(const dblk_t *db, mac_ether_offload_info_t *info)
{
...
{
	unpack_tunpktinfo_inline(db, info);
}
...
mac_ether_offload_info(const mblk_t *pkt, mac_ether_offload_info_t *info)
{
	...
	if (db->db_pktinfo.t_tuntype == METT_NONE) {
		unpack_pktinfo_inline(db, info);
		if (!MEOI_IS_FULL(info))
			err = mac_partial_offload_info(pkt, 0, info);
	} else {
		unpack_tunpktinfo_inline(db, info);
		if (!MEOI_IS_FULLTUN(info))
    ...
    mac_ether_offload_info_t *inner_info)
{
	...
	if (inner_info != NULL) {
		...
		if (is_tun) {
			...
			if (!MEOI_IS_FULLTUN(outer_target))
				return (-1);

			inner_info->meoi_len = outer_target->meoi_len -
			    inner_offset;
			unpack_pktinfo_inline(db, inner_info);
			if (!MEOI_IS_FULL(inner_info))
				err = mac_partial_offload_info(pkt,
				    inner_offset, inner_info);
			if (err != 0)
