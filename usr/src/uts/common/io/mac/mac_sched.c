...
check_again:
		...
		if (!worker_live && mac_srs->srs_first != NULL) {
			 ...
			 */
			if (mac_srs_is_latency_opt(mac_srs)) {
				mac_srs->srs_drain_func(mac_srs,
				    SRS_POLL_PROC);
				if (!(mac_srs->srs_state & SRS_PAUSE) &&
...
static bool
mac_pkt_is_flow_match(flow_entry_t *, const mac_flow_match_t *, mblk_t *);
...
mac_pkt_is_flow_match_i(flow_entry_t *flent, const mac_flow_match_t *match,
    mblk_t *mp)
{
	ASSERT3P(flent, !=, NULL);
	ASSERT3P(mp, !=, NULL);
...
mac_pkt_is_flow_match(flow_entry_t *flent, const mac_flow_match_t *match,
    mblk_t *mp)
{
	return (mac_pkt_is_flow_match_i(flent, match, mp));
}
 ...
 * to_class belong to the same flow. This allows us to only classify the first
    ...
    mac_pkt_list_t *to_class, mac_pkt_list_t *classed, const bool single_flow)
{
	...
	if (single_flow) {
		const bool is_match = mac_pkt_is_flow_match_i(enode->ften_flent,
	...
	while (curr != NULL) {
		...
		if (is_match) {
			*to_curr = curr->b_next;
			...
			const size_t lsz = msgdsize(curr);
    ...
    size_t *policed_bytes)
{
	...
	while (curr != NULL && !early_exit) {
		mblk_t **to_curr = (prev != NULL) ? &prev->b_next :
		    &to_class->mpl_head;
		const bool is_match = flow_hit || mac_pkt_is_flow_match_i(
    ...
    const mac_soft_ring_set_state_t proc_type)
{
	...
	if (is_tx) {
		...
		const mac_srs_tx_t *srs_tx = &give_to->srs_tx;
		...
		if (error == 0) {
			...
			mac_strip_chain_hints(pkts.mpl_head);
			(void) srs_tx->st_func(give_to, pkts.mpl_head, hint, 0,
			    NULL);
...
{
	mac_pkt_list_append_pkt(into, mp, msgdsize(mp));
}
