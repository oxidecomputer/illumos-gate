    ...
    const flow_entry_t *flent)
{
	...
	if (mac_srs_is_tx(srs)) {
		const mac_tx_stats_t *ts = &srs->srs_tx.st_stat;
		if (is_logical) {
			/*
		...
		const mac_rx_stats_t *rs = &srs->srs_rx.sr_stat;
		if (is_logical) {
...
flow_stat_update(kstat_t *ksp, int rw)
{
	...
	/*
	 * This read ensures that we're looking at a consistent set of SRSes,
	 * and that datapath setup/reconfiguration won't destroy an SRS and fold
	 * its stats back into the flent while we're here. This applies to even
	 * the cases where we have only complete SRSes on a link flow --
	 * add/remove/movement of rings between groups will alter the set of
	 * complete SRSes visible to this flent via fe_rx_srs_cnt.
	 */
	rw_enter(&fep->fe_srs_list_lock, RW_READER);
	...
	for (mac_soft_ring_set_t *curr = list_head(&fep->fe_srs_list);
	    curr != NULL; curr = list_next(&fep->fe_srs_list, curr)) {
		flow_stat_fill(&flow_stats, curr, fep);
	}

	rw_exit(&fep->fe_srs_list_lock);
    ...
    void *client_cookie, uint_t type, flow_entry_t **flentp)
{
	...
	if (flent == NULL) {
		...
		rw_init(&flent->fe_srs_list_lock, NULL, RW_DEFAULT, NULL);
		list_create(&flent->fe_srs_list, sizeof (mac_soft_ring_set_t),
		    offsetof(mac_soft_ring_set_t, srs_in_fe));
...
mac_flow_destroy(flow_entry_t *flent)
{
	...
	VERIFY(list_is_empty(&flent->fe_srs_list));
	...
	rw_destroy(&flent->fe_srs_list_lock);
	list_destroy(&flent->fe_srs_list);
