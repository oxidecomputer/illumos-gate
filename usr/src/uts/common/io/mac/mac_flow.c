...
mac_flow_fill_tree_match(flow_entry_t *flent)
{
	if (flent->fe_match == flow_transport_lport_match) {
		...
		flent->fe_ft_match.arg.mfm_list = mac_flow_match_list_create(2);
		flent->fe_ft_match.arg.mfm_list->mfml_match[0].mfm_type =
		    MFM_IPPROTO;
		flent->fe_ft_match.arg.mfm_list->mfml_match[0].arg.mfm_ipproto =
		    flent->fe_flow_desc.fd_protocol;
		flent->fe_ft_match.arg.mfm_list->mfml_match[1].mfm_type =
		    MFM_L4_LOCAL;
		flent->fe_ft_match.arg.mfm_list->mfml_match[1].arg.mfm_l4addr =
...
/*
 * Remove the `i`th element from a `mac_flow_match_t` of type `MFM_ALL` or
 * `MFM_ANY`.
 */
void
...
/*
 * Perform a recursive clone of a `mac_flow_match_t`.
 */
mac_flow_match_t
 ...
 */
void
mac_flow_specialise_match(mac_flow_match_t *ma, const bool is_tx)
{
	switch (ma->mfm_type) {
	case MFM_L4_REMOTE:
	...
	case MFM_ANY: {
		...
		for (size_t i = 0; i < in_list->mfml_size; i++) {
			mac_flow_specialise_match(in_list->mfml_match + i,
			    is_tx);
		}
		break;
	}
