...
static mblk_t *
mac_sw_cksum_inner(mblk_t *mp, mac_emul_t emul, const uint32_t encap_len,
    const mac_ether_offload_info_t *meoi)
{
	...
	uint32_t frame_offset = encap_len + meoi->meoi_l2hlen;
	while (target_mp != NULL && frame_offset >= MBLKL(target_mp)) {
		size_t seglen = MBLKL(target_mp);
		size_t n = MIN(seglen, frame_offset);
	 ...
	 * target_mp.
	 */
	const size_t hdr_len_reqd = (frame_offset + meoi->meoi_l3hlen) +
	...
	if (do_v4_cksum) {
		 ...
		 */
		ipha_t *ipha = (ipha_t *)(target_mp->b_rptr +
		    meoi->meoi_l2hlen + frame_offset);
	...
	if (do_ulp_cksum && meoi->meoi_l4proto == IPPROTO_SCTP) {
		...
		const uint_t ulp_off = meoi->meoi_l2hlen + meoi->meoi_l3hlen +
		    frame_offset;
		sctp_hdr_t *sctph = (sctp_hdr_t *)(target_mp->b_rptr + ulp_off);

		sctph->sh_chksum = 0;
		sctph->sh_chksum = sctp_cksum(target_mp, ulp_off);
		if (is_outer) {
			flags &= ~HCK_FULLCKSUM;
	...
	if (do_ulp_cksum && ulp_prefer_partial) {
		 ...
		 */
		if (stuff + sizeof (uint16_t) + frame_offset > MBLKL(target_mp)) {
			err = "partial csum request is out of bounds";
			goto bail;
		}

		uchar_t *ipp = (uchar_t *)(target_mp->b_rptr +
		    meoi->meoi_l2hlen + frame_offset);
		...
		*up = 0;
		const uint16_t cksum =
		    ~IP_CSUM_PARTIAL(mp, start + encap_len + meoi->meoi_l2hlen,
		    partial);
 ...
 *
 * To correctly handle tunnelled packets, frames are processed from the inside
    ...
    const mac_ether_offload_info_t *inner_info)
{
	...
	if (encap_len != 0)
		mp = mac_sw_cksum_inner(mp, emul, encap_len, inner_info);
	if (mp != NULL)
		mp = mac_sw_cksum_inner(mp, emul, 0, outer_info);
    ...
    const mac_ether_offload_info_t *inner_info)
{
	 ...
	 * One difficulty is that when packets are tunnelled and in a single
	 ...
	 * ensures tha inner TCP etc. cannot be 4B aligned, and vice-versa.
	 */
	mblk_t *omp_inner = omp;
	mblk_t *omp_parent = NULL;
	...
	/* Walk past encapsulation to reach the inner L3/L4 (may just be omp) */
	...
	/*
	 * Ensure the innermost L3/L4 headers are contiguous. The IP header is
	 * used only for the benefit of DTrace SDTs, whereas the TCP header is
	 * actively read. This allows for L2 and L3/L4 to be separated as they
	 * are when mac_add_vlan_tag is in play, which prepends a new mblk in
	 * front containing the amended Ethernet header.
	 * For simplicity, also copy any preceding bytes from omp_inner. Most
	 * clients (IP, viona) will setup well-behaved mblks.
	 */
	const size_t hdr_len_reqd = offset + oiphlen + otcphlen;
	if (MBLKL(omp_inner) < hdr_len_reqd) {
		...
		if (omp_parent == NULL) {
			...
			omp_parent->b_cont = tmp;
		}

		freemsg(omp_inner);
		omp_inner = tmp;
	}
	oiph = (void *)(omp_inner->b_rptr + offset);
	offset += oiphlen;
	...
	if ((ocsum_flags & HCK_TX_FLAGS) && (emul & MAC_HWCKSUM_EMULS)) {
		...
		nhdrmp->b_next = NULL;
		nhdrmp = mac_sw_cksum(nhdrmp, emul, encap_len, outer_info,
		    inner_info);
...
mac_hw_emul(mblk_t **mp_chain, mblk_t **otail, uint_t *ocount, mac_emul_t emul)
{
	...
	for (mblk_t *mp = *mp_chain; mp != NULL; ) {
		 ...
		 * e.g., v4 checksum offload on ICMP packets, or tunnelled
		...
		/*
		 * Compute offset to inner packet if tunnelled.
		 ...
		 */
		if (len < inner_frame_offset + l2len) {
		...
		if ((flags & HW_LSO) && (emul & MAC_LSO_EMUL)) {
			...
			mac_sw_lso(mp, emul, &tmphead, &tmptail,
			    &tmpcount, encap_len, &outer_info, &inner_info);
			if (tmphead == NULL) {
		    ...
		    (emul & MAC_HWCKSUM_EMULS)) {
			tmp = mac_sw_cksum(mp, emul, encap_len, &outer_info,
			    &inner_info);
			if (tmp == NULL) {
