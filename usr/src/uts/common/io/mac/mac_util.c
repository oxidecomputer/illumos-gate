 ...
 * out (i.e., any outer L4 packet cksums are reliant on correct inner
...
mac_sw_cksum(mblk_t *mp, mac_emul_t emul, const uint32_t encap_len)
{
	...
	/*
	 * TODO: resolve and rewrite. Must handle in order: inner -> outer
	 *
	 * Really need that flagshift/unshift to make this easier.
	 * Probably want to fail packets with *inner* flags but no tuntype?
	 */
	...
	if (etype == ETHERTYPE_IP) {
		...
		/* We always update the ULP checksum flags. */
		if ((layer_flags & HCK_FULLCKSUM) && (emul & MAC_HWCKSUM_EMULS)) {
			if (is_outer) {
				flags &= ~HCK_FULLCKSUM;
				flags |= HCK_FULLCKSUM_OK;
			} else {
