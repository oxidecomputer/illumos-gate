    ...
    const mac_ether_offload_info_t *meoi)
{
	...
	if (do_ulp_cksum && ulp_prefer_partial) {
		...
		/*
		 * For tunnelled packets, the above should not be set (and would
		...
		if (encap_len != 0 || meoi->meoi_tuntype != METT_NONE) {
			start = encap_len + meoi->meoi_l2hlen +
			    meoi->meoi_l3hlen;
			stuff = l3_off + meoi->meoi_l3hlen;
			switch (meoi->meoi_l4proto) {
			case IPPROTO_TCP:
			...
			ASSERT3U(end, >, start);
			start += meoi->meoi_l2hlen;
			stuff += l3_off;
		}

		// XXX(kyle)
		cmn_err(CE_NOTE, "partial case -- I have %u bytes(%u), start=%u stuff=%u l3=%u l4=%u",
		    (uint_t)(MBLKL(target_mp)), encap_len, start, stuff, meoi->meoi_l3proto, meoi->meoi_l4proto);
		...
		*up = 0;
		const uint16_t cksum = IP_CSUM(mp, start, partial);
    ...
    const mac_ether_offload_info_t *inner_info)
{
	...
	if (mp != NULL && (flags & HCK_OUTER_TX_FLAGS) != 0) {
    ...
    const mac_ether_offload_info_t *inner_info)
{
	...
	uint32_t offset = encap_len + oehlen;
	oiph = (void *)(omp->b_rptr + offset);
	offset += oiphlen;
	otcph = (tcph_t *)(omp->b_rptr + offset);
	offset += otcphlen;
	...
	for (uint_t i = 0; i < nsegs; i++) {
		...
		/*
		 * Setup partial checksum offsets for non-tunnelled packets.
		 * mac_sw_cksum will figure precise offsets out for tunnelled
		 * packets, as we may have two partial
		 */
		if (!is_tun && tcp_csum_partial) {
