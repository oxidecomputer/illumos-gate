...
/*
 * Copyright (c) 1998 by Oxide Computer, Inc.
 * All rights reserved.
 */
...
typedef struct spidev_transfer {
    ...
    /*
     * delay introduced after this transfer but before chipselect change, next
     * transfer, or completion of transaction.
     */
    uint16_t    delay_usec;

    /* Set to zero for controller default */
    ...
    /*
     * Delay introduced between chipselect deassert and reassert when cs_change
     * is non-zero and this is not the last transfer in the transaction.
     */
    uint8_t     cs_change_delay_usec;

    /* Delay introduced between words sent in this transfer. */
    uint8_t     word_delay_usec;

    /* width of data bus (in bits) used for writing */
    uint8_t     tx_width;
    /* width of data bus (in bits) used for reading */
    uint8_t     rx_width;
} spidev_transfer_t;
