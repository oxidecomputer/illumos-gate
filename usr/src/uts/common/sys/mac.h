...
typedef enum mac_ring_query_res {
	/* The requested value was successfully written. */
	MRQR_OK = 0,
	/*
	 * The MAC resource does not recognise or support the given
	 * mac_ring_query_t.
	 */
	MRQR_UNRECOGNISED,
	/* The output buffer is too small to contain the requested value. */
	MRQR_TOO_SMALL,
} mac_ring_query_res_t;

typedef enum mac_ring_query {
	/*
	 ...
	 *
	 * The output type is a bool.
	 */
	MRQ_IS_HW,
} mac_ring_query_t;

typedef	mblk_t			*(*mac_receive_t)(void *, size_t);
typedef	mac_ring_query_res_t	(*mac_ring_querier_t)(const void *,
				    mac_ring_query_t, void *, size_t);
typedef	void			(*mac_ring_await_t)(void *);
