...
struct mac_soft_ring_set_s {
	...
	/*
	 * Doubly-linked list of all SRSes sharing our srs_flent.
	 */
	list_node_t	srs_in_fe;	/* fe_srs_list_lock */
	 ...
	 *
	 * Use is protected by srs_logical_lock on the head of the list. Stats
	 * collection is why we require the use of an RW lock. Without this
	 * use case, the MAC perimeter would suffice.
	 *
	 * A write lock must only be acquired by a thread which holds the MAC
	 * perimeter and has quiesced the SRS. This should be held only for the
	 * duration of operations which modify (add, remove, resize) the list
	 * or its contents.
	 */
	krwlock_t		srs_logical_lock;
	mac_soft_ring_set_t	*srs_logical_next;

	mac_soft_ring_set_t	*srs_complete_parent;	/* WO */
...
extern void mac_update_srs_count(mac_soft_ring_set_t *, const uint32_t);
...
extern mac_tx_func_t mac_tx_get_func(mac_tx_srs_mode_t);
...
extern void mac_srs_bw_lock(const mac_soft_ring_set_t *);

extern void mac_srs_bw_unlock(const mac_soft_ring_set_t *);
