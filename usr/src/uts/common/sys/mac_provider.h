 ...
 * required checksum on a tunnelled packet, even over IPv6 -- see RFC 6935/6936.
 *
 ...
 * missing values the `mac_ether_offload_info` (outer) or
 * `mac_ether_fullpktinfo` (outer and inner) functions. NIC drivers which do not
 * expose tunnel aware offloads may request only that the outer information be
 * filled.
 *
...
#define	MEOI_IS_FULL(m)	(((m)->meoi_flags & MEOI_FULL) == MEOI_FULL)
#define	MEOI_IS_FULLTUN(m)	\
	(((m)->meoi_flags & MEOI_FULLTUN) == MEOI_FULLTUN)
...
extern int mac_partial_offload_info(const mblk_t *, size_t,
    mac_ether_offload_info_t *);
extern int mac_partial_tun_info(const mblk_t *, size_t,
    mac_ether_offload_info_t *);
extern int mac_ether_offload_info(const mblk_t *, mac_ether_offload_info_t *);
extern int mac_ether_fullpktinfo(const mblk_t *, mac_ether_offload_info_t *,
    mac_ether_offload_info_t *);
extern int mac_ether_set_fullpktinfo(mblk_t *, const mac_ether_offload_info_t *,
    const mac_ether_offload_info_t *);
extern void mac_ether_clear_pktinfo(mblk_t *);

extern void unpack_pktinfo(const dblk_t *db, mac_ether_offload_info_t *info);
extern void unpack_tunpktinfo(const dblk_t *db, mac_ether_offload_info_t *info);

extern mac_ether_tun_type_t mac_ether_tun_type(const mblk_t *);
extern int mac_ether_push_tun(mblk_t *, mac_ether_tun_type_t);
extern int mac_ether_shift_tun(mblk_t *, mac_ether_tun_type_t);
extern int mac_ether_push_tuninfo(mblk_t *, const mac_ether_offload_info_t *);
extern int mac_ether_pop_tun(mblk_t *);

extern uint32_t mac_hcksum_flags_shift_out(uint32_t);
extern uint32_t mac_hcksum_flags_shift_in(uint32_t);
