...
#define	ZEN_FCH_IOAPIC_APICID		0xf0
#define	ZEN_IOHC_IOAPIC_APICID		0xf1
 ...
 */
uint32_t apic_intr_max_procid = INTR_MAX_PROCID;
...
apic_probe_raw(const char *modname)
{
	...
	zen_populate_iommu_ioapic_device_ids(apic_io_iommu_devid, MAX_IO_APIC);
	apic_io_id[0] = ZEN_FCH_IOAPIC_APICID;
	apic_physaddr[0] = ZEN_PHYSADDR_FCH_IOAPIC;
	apicioadr[0] = (void *)mapin_ioapic(apic_physaddr[0], APIC_IO_MEMLEN,
	    PROT_READ | PROT_WRITE);

	apic_io_id[1] = ZEN_IOHC_IOAPIC_APICID;
...
{
	return (apic_cpu_in_range(cpu) &&
	    apic_get_localapicid(cpu) <= apic_intr_max_procid);
}
...
apic_nmi_intr(caddr_t arg __unused, caddr_t arg1 __unused)
{
	 ...
	 * response to an external assertion of a pin like NMI_SYNCFLOOD_L.  If
	 * so, we must clear the indicator flag and signal EOI to the fabric in
	 * order to receive subsequent such NMIs.
	 */
	zen_fabric_nmi_eoi();
...
{
	bool suppress_broadcast_eoi = apic_directed_EOI_supported() == 0;
	apic_intrmap_ops_t *ops = psm_vt_ops;

	/*
	 * In the Oxide architecture, we unconditionally enable the x2APIC on
	 * all CPUs.
	 *
	 * The Intel SDM says that to enable x2 mode, you must have already set
	 * up the IOMMU and interrupt remapping, but empirically we know this is
	 * not strictly true on both Intel and AMD hardware: x2APIC mode works
	 * fine, and IPIs and local interrupts (e.g., the LAPIC timer) work with
	 * no additional configuration.  Similarly, we can handle device
	 * interrupts, as long as we direct them to cores with (x)APIC IDs that
	 * are addressible with 8-bit (x)APIC IDs.
	 *
	 * However, delivering device interrupts to cores with APIC IDs that do
	 * not fit into 8 bits requires interrupt remapping, which is provided
	 * by the IOMMU.
	 *
	 * In other words, to use the APIC in x2APIC mode, and we have
	 * processors with APIC IDs greater than 254, then we need to start the
	 * IOMMU and configure and enable interrupt remapping to deliver
	 * interrupts generated by peripherals to those cores.
	 *
	 * With the AMD Turin microarchitecture, we have 1P systems with core
	 * counts up to 256; with threading, this turns into 512 logical
	 * processors.  We want to be able to deliver peripheral interrupts to
	 * those CPUs, so this support is essential.
	 *
	 * As an additional wrinkle, the x2APIC ID space is sparse, as it
	 * encodes socket/package/node and so on, so we have to maintain two
	 * separate values: the number of processors in the system, and the
	 * largest APIC ID we can direct interrupts to; all-bits-1 is the APIC
	 * broadcast address, so by default, we select 254 for the latter, but
	 * if we know that interrupt remapping has been enabled, we can widen
	 * this to 0xFFFF_FFFE, which covers the full 32-bit APIC ID space.
	 */
	if (ops == NULL || ops->apic_intrmap_init(apic_mode) != DDI_SUCCESS)
		return;

	apic_vt_ops = psm_vt_ops;
	apic_intr_max_procid = UINT32_MAX - 1;

	/*
	 * If supported, we leverage the interrupt remapping engine to suppress
	 * broadcast EOI.  In that case, we must send the directed EOI with the
	 * directed-EOI handler.
	 *
	 * This is only meaningful for level-triggered interrupts generated by
	 * devices attached to an IOAPIC, of which we presently have none.
	 */
	apic_vt_ops->apic_intrmap_enable(suppress_broadcast_eoi);
	if (suppress_broadcast_eoi)
