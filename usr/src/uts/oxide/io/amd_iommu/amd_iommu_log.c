...
static const char *
amd_iommu_hwerr(uint8_t type)
{
	static const char *errs[] = {
	    "Reserved",  "Master Abort", "Target Abort", "Data Error"
	};

	if (type < ARRAY_SIZE(errs))
		return (errs[type]);
...
get_illegal_req(uint8_t type, uint8_t TR)
{
	const char *errs[] = {
	    ...
	    "OR Posted write to addr transaltion range with HtAtsResv=1",

	    "Read request or non-posted write in SysMgt with "
	    "device SysMgt=10b or 0xb OR Read request or non-posted write "
	    "in addr translation range with HtAtsResv=1",

	    "Posted write to Interrupt/EOI Range for device with IntCtl=00b",
	    "Posted write to reserved Interrupt Address Range",

	    "Transaction to SysMgt when SysMgt=11b OR "
	    "transaction to Port-IO when IoCtl=10b while V=1 TV=0",
	};
	const char *errs_tr[] = {
	    "Translation I=0/V=0/V=1&&TV=0",
	    "Translation INTR/Port-IO/SysMgt OR "
	    "translation when SysMgt=11b/Port-IO and IOCTL=10b and V=1 && TV=0",
	};

	if (type < ARRAY_SIZE(errs)) {
		if (TR == 1) {
			if (type < ARRAY_SIZE(errs_tr))
				return (errs_tr[type]);
			return ("Reserved");
		}
		return (errs[type]);
...
devtab_illegal_entry(amd_iommu_t *iommu, uint32_t *event)
{
	...
	cmn_err(CE_WARN, "illegal device table entry: amd_iommu%d idx = %d. "
	    "Illegal device table entry deviceid=%u, %s request, %s %s "
	    "transaction, %s request, virtual address = %p",
	    iommu->ai_instance, iommu->aiomt_idx,
	    deviceid,
