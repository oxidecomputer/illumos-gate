 ...
 * given PCIe signalling rate, along with that rate's preset search mask.
 *
 * The per-lane settings in LC_BEST_EQ_SETTINGS and LC_CNTL5 report on the
 * single lane selected by DEBUG_LANE_EN in the parent core's LC_DEBUG_CNTL,
 * at the rate selected within each register. Reading them therefore
 * requires writing those selectors, so we save the three registers we
 * disturb and restore them afterwards in an attempt to leave the hardware
 * as we found it.
 */
int
turin_pcie_port_eq(zen_pcie_port_t *port, pcie_link_speed_t speed,
    uint32_t nlanes, pcie_eq_t *eq)
{
	...
	dbg_reg = turin_pcie_core_reg(pc, D_PCIE_CORE_LC_DEBUG_CNTL);
	best_reg = turin_pcie_port_reg(port, D_PCIE_PORT_LC_BEST_EQ);
	ctl5_reg = turin_pcie_port_reg(port, D_PCIE_PORT_LC_CTL5);
	mask_reg = turin_pcie_port_reg(port, D_PCIE_PORT_LC_PRST_MASK_CTL);
	...
	(void) zen_pcie_port_dbg_val_by_name(port,
	    "PCIEPORT::PCIE_LC_PRESET_MASK_CNTL", ZPCS_LINK_UP, &linkup_mask,
	    &linkup_ts);

	switch (speed) {
	case PCIE_LINK_SPEED_8:
		rate = PCIE_PORT_LC_BEST_EQ_RATE_8P0;
		eq->peq_mask = PCIE_PORT_LC_PRST_MASK_CTL_GET_MASK_8GT(mask);
		eq->peq_mask_linkup =
		    PCIE_PORT_LC_PRST_MASK_CTL_GET_MASK_8GT(linkup_mask);
		break;
	case PCIE_LINK_SPEED_16:
		rate = PCIE_PORT_LC_BEST_EQ_RATE_16P0;
		eq->peq_mask = PCIE_PORT_LC_PRST_MASK_CTL_GET_MASK_16GT(mask);
		eq->peq_mask_linkup =
		    PCIE_PORT_LC_PRST_MASK_CTL_GET_MASK_16GT(linkup_mask);
		break;
	case PCIE_LINK_SPEED_32:
		rate = PCIE_PORT_LC_BEST_EQ_RATE_32P0;
		eq->peq_mask = PCIE_PORT_LC_PRST_MASK_CTL_GET_MASK_32GT(mask);
		eq->peq_mask_linkup =
		    PCIE_PORT_LC_PRST_MASK_CTL_GET_MASK_32GT(linkup_mask);
		break;
	default:
		return (EINVAL);
	}
	...
	for (uint32_t lane = 0; lane < nlanes; lane++) {
		...
		dbg = PCIE_CORE_LC_DEBUG_CNTL_SET_DEBUG_LANE_EN(save_dbg,
		    1U << lane);
		...
		pel->pel_flags |= PCIE_EQ_LANE_F_USE_PRESET_VALID;
		if (PCIE_PORT_LC_CTL5_GET_USE_PRESET(ctl5) != 0)
			pel->pel_flags |= PCIE_EQ_LANE_F_USE_PRESET;
	}
...
int
turin_pcie_port_set_preset_mask(zen_pcie_port_t *port, pcie_link_speed_t speed,
    uint32_t mask)
{
	...
	switch (speed) {
	case PCIE_LINK_SPEED_8:
		val = PCIE_PORT_LC_PRST_MASK_CTL_SET_MASK_8GT(val, mask);
		eq_reg = turin_pcie_port_reg(port, D_PCIE_PORT_LC_EQ_CTL_8GT);
		eq_val = zen_pcie_port_read(port, eq_reg);
		eq_val = PCIE_PORT_LC_EQ_CTL_8GT_SET_REDO_EQ(eq_val, 1);
		break;
	case PCIE_LINK_SPEED_16:
		val = PCIE_PORT_LC_PRST_MASK_CTL_SET_MASK_16GT(val, mask);
		eq_reg = turin_pcie_port_reg(port, D_PCIE_PORT_LC_EQ_CTL_16GT);
		eq_val = zen_pcie_port_read(port, eq_reg);
		eq_val = PCIE_PORT_LC_EQ_CTL_16GT_SET_REDO_EQ(eq_val, 1);
		break;
	case PCIE_LINK_SPEED_32:
		val = PCIE_PORT_LC_PRST_MASK_CTL_SET_MASK_32GT(val, mask);
		eq_reg = turin_pcie_port_reg(port, D_PCIE_PORT_LC_EQ_CTL_32GT);
		eq_val = zen_pcie_port_read(port, eq_reg);
		eq_val = PCIE_PORT_LC_EQ_CTL_32GT_SET_REDO_EQ(eq_val, 1);
		break;
	default:
		return (EINVAL);
	}
    ...
    const uint16_t reginst)
{
	...
	case SMN_UNIT_IOMMUMMIO:
		/*
		 * Similar the the SMN_UNIT_IOMMUL2 case above, the IOMMUMMIO
		 * registers are only available in the larger IOHC instances.
		 */
		VERIFY3U(ioms->zio_iohctype, ==, ZEN_IOHCT_LARGE);
		reg = turin_iommummio_smn_reg(iohcnum, def, reginst);
		break;
	default:
...
smn_reg_t
turin_iommu_mmio_reg(const zen_ioms_t *const ioms, const smn_reg_def_t def)
{
	if (def.srd_unit != SMN_UNIT_IOMMUMMIO) {
		cmn_err(CE_PANIC, "invalid SMN register type %d for IOMMU MMIO",
		    def.srd_unit);
	}
	return (turin_ioms_reg(ioms, def, 0));
}
