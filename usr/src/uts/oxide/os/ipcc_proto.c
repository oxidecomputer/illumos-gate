...
uint_t ipcc_syncpoll_timeout_ms = 0; /* 0 => disabled by default */
...
ipcc_pkt_send(uint8_t *pkt, size_t len, const ipcc_ops_t *ops, void *arg)
{
	 ...
	 * reason, so we flush the channel at the start of each command, and on
	 * every resend of a failed command.  We believe that the delays we have
	 * selected and artificially inserted into that process (see, e.g.,
	 * `espi_oob_flush`) are sufficient that any data in the distant end's
	 * output queue but not yet on the channel will have time to drain, and
	 * will thus be flushed.
	 *
	 ...
	 * this, we may attempt to "synchronize" the channel to the start of a
	 * frame before sending, avoiding a suprious receive failure for a
	 * known case we have seen before.
	 *
	 * However, the cost of this is significant: there is a non-trivial
	 * timeout whenever we invoke `ipcc_sync`, and in the common case,
	 * where there is nothing in the channel, we incur that on every packet
	 * send.  Thus, by default, the "timeout" is zero, meaning we don't
	 * try to synchronize.
	 */
...
resend:
	 ...
	 * so on.  Since this invocation of `ipcc_command_locked` corresponds
	 * to the start of a new request, with a new sequence number, any data
	 * currently in the channel is irrelevant to that request, and can be
	 * safely discarded.  Similarly, each retransmission will result in a
	 * fresh response being generated by the SP, so it is safe to discard
	 * pending data.
	 */
