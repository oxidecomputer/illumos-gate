...
/*
 * This implements RAS support on AMD Zen.
 */
...
identify_bank(uint_t bank)
{
	...
	uint64_t ipid = read_bank_msr(bank, ZEN_RAS_MSR_IPID);
...
{
	const uint64_t mask = rdmsr(ZEN_RAS_MCA_CTL_MASK_MSR_BASE + bank);
	const uint64_t value = bitset64(mask, bit, bit, 1);
	wrmsr(ZEN_RAS_MCA_CTL_MASK_MSR_BASE + bank, value);
}
...
zen_ras_init(void)
{
	...
	for (uint_t bank = 0; bank < nbanks; bank++) {
		...
		if (is_setb(cfg, ZEN_RAS_CFG_TRANSPARENT_LOGGING_SUPTD))
			cfg = delb(cfg, ZEN_RAS_CFG_TRANSPARENT_LOGGING_EN);
		...
		if (bank > ZEN_RAS_MAX_MCAX_BANKS) {
			cfg = delb(cfg, ZEN_RAS_CFG_MCAX_EN);
		...
		case ZEN_RBT_LS:
			bank_mask_set(bank, ZEN_RAS_MASK_LS_SYS_RD_DATA_LD);
			bank_mask_set(bank, ZEN_RAS_MASK_LS_SYS_RD_DATA_SCB);
			bank_mask_set(bank, ZEN_RAS_MASK_LS_SYS_RD_DATA_WCB);
			break;
		case ZEN_RBT_IF:
			/* These appear to be set by HW/FW; take no chances. */
			bank_mask_set(bank, ZEN_RAS_MASK_IF_L2_BTB_MULTI);
			bank_mask_set(bank, ZEN_RAS_MASK_IF_L2_TLB_MULTI);
			break;
		case ZEN_RBT_L2:
			bank_mask_set(bank, ZEN_RAS_MASK_L2_HWA);
			break;
		case ZEN_RBT_FP:
			bank_mask_set(bank, ZEN_RAS_MASK_FP_HWA);
			break;
		case ZEN_RBT_CS:
			bank_mask_set(bank, ZEN_RAS_MASK_CS_FTI_ADDR_VIOL);
			break;
		case ZEN_RBT_L3:
			bank_mask_set(bank, ZEN_RAS_MASK_L3_HWA);
			break;
		case ZEN_RBT_NBIO:
			bank_mask_set(bank, ZEN_RAS_MASK_NBIO_PCIE_SB);
			bank_mask_set(bank, ZEN_RAS_MASK_NBIO_PCIE_ERR_EVT);
			break;
		default:
			break;
		}
