...
zen_fabric_ioms_pcie_init(zen_ioms_t *ioms)
{
	...
	for (uint8_t coreno = 0; coreno < ioms->zio_npcie_cores; coreno++) {
		...
		/*
		 * Calculate the unit ID for this core's SDP port, which will
		 * later be programmed into PCIECORE::PCIE_SDP_CTRL. In all
		 * supported microarchitectures, PCIe ports are assigned
