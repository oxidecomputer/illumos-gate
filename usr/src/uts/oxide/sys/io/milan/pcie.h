...
extern uint8_t milan_iohc_n_pcie_cores(const uint8_t);
    ...
    const uint8_t coreno)
{
	...
	/*
	 * Note: We'd like to use the macro MILAN_IOHC_PER_IODIE but it is
	...
	ASSERT3U(iohc32, <, iohc_per_iodie);
	ASSERT0(def.srd_reg & ~PCIE_CORE_SMN_REG_MASK);

#ifdef	DEBUG
	const uint32_t nents = milan_iohc_n_pcie_cores(iohcno);
...
#ifdef	DEBUG
	const uint32_t ncores = (const uint32_t)milan_iohc_n_pcie_cores(iohcno);
