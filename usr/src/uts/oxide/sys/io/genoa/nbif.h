 ...
 */
#define	GENOA_MAX_NBIO			2
#define	GENOA_NBIO_MAX_NBIF		3
#define	GENOA_NBIO_MAX_NBIF_ALT		2
/*
...
genoa_nbif_func_smn_reg(const uint8_t nbiono, const smn_reg_def_t def,
    const uint8_t nbifno, const uint8_t devno, const uint8_t funcno)
{
	 ...
	 * with DEBUG enabled. This must be in sync with genoa_nbif_data in
...
static inline smn_reg_t
genoa_nbif_smn_reg(const uint8_t nbiono, const smn_reg_def_t def,
...
static inline smn_reg_t
genoa_nbif_alt_smn_reg(const uint8_t nbiono, const smn_reg_def_t def,
    const uint8_t nbifno, const uint16_t reginst)
{
	const uint32_t nbio32 = (const uint32_t)nbiono;
	const uint32_t nbif32 = (const uint32_t)nbifno;
	...
	ASSERT3U(nbio32, <, GENOA_MAX_NBIO);
	ASSERT3U(nbif32, <, GENOA_NBIO_MAX_NBIF_ALT);
	ASSERT3U(nents, >, reginst32);
	ASSERT0(def.srd_reg & SMN_APERTURE_MASK);
