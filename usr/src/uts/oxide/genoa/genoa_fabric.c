 ...
 */
void
genoa_fabric_nbio_init(zen_nbio_t *nbio)
{
	nbio->zn_sst_start = 0;
	nbio->zn_sst_count = 1;
	/* NBIO1 has a bonus SST instance */
	if (nbio->zn_num == GENOA_NBIO_BONUS_SST)
		nbio->zn_sst_count++;
}
 ...
 */
int
genoa_pcie_port_eq(zen_pcie_port_t *port, pcie_link_speed_t speed,
    uint32_t nlanes, pcie_eq_t *eq)
{
	...
	dbg_reg = genoa_pcie_core_reg(pc, D_PCIE_CORE_LC_DBG_CTL);
	best_reg = genoa_pcie_port_reg(port, D_PCIE_PORT_LC_BEST_EQ);
	ctl5_reg = genoa_pcie_port_reg(port, D_PCIE_PORT_LC_CTL5);
	mask_reg = genoa_pcie_port_reg(port, D_PCIE_PORT_LC_PRST_MASK_CTL);
...
int
genoa_pcie_port_set_preset_mask(zen_pcie_port_t *port, pcie_link_speed_t speed,
    uint32_t mask)
{
	...
	case PCIE_LINK_SPEED_8:
		...
		eq_reg = genoa_pcie_port_reg(port, D_PCIE_PORT_LC_EQ_CTL_8GT);
	...
	case PCIE_LINK_SPEED_16:
		...
		eq_reg = genoa_pcie_port_reg(port, D_PCIE_PORT_LC_EQ_CTL_16GT);
		eq_val = zen_pcie_port_read(port, eq_reg);
		eq_val = PCIE_PORT_LC_EQ_CTL_16GT_SET_REDO_EQ(eq_val, 1);
		break;
	case PCIE_LINK_SPEED_32:
		val = PCIE_PORT_LC_PRST_MASK_CTL_SET_MASK_32GT(val, mask);
		eq_reg = genoa_pcie_port_reg(port, D_PCIE_PORT_LC_EQ_CTL_32GT);
		eq_val = zen_pcie_port_read(port, eq_reg);
...
static smn_reg_t
genoa_nbio_reg(const zen_nbio_t *const nbio, const smn_reg_def_t def,
    const uint16_t reginst)
{
...
void
genoa_fabric_nbio_arbitration(zen_nbio_t *nbio)
{
	smn_reg_t reg;
...
void
genoa_fabric_nbio_clock_gating(zen_nbio_t *nbio)
{
	smn_reg_t reg;
	uint32_t val;
	...
	for (uint_t i = 0; i < ARRAY_SIZE(sdpmux_regs); i++) {
		reg = genoa_nbio_reg(nbio, sdpmux_regs[i], 0);
		val = zen_nbio_read(nbio, reg);
	...
	for (uint16_t i = nbio->zn_sst_start;
	    i < nbio->zn_sst_start + nbio->zn_sst_count; i++) {
		reg = genoa_nbio_reg(nbio, D_SST_CLOCK_CTL, i);
		val = zen_nbio_read(nbio, reg);
		...
		reg = genoa_nbio_reg(nbio, D_SST_SION_WRAP_CFG_GCG_LCLK_CTL, i);
		val = zen_nbio_read(nbio, reg);
