...
genoa_pcie_core_reg(const zen_pcie_core_t *const pc, const smn_reg_def_t def)
{
	...
	// 	reg = genoa_pcie_rsmu_smn_reg(ioms->zio_num, def,
	// 	    pc->zpc_coreno);
	// 	break;
...
#if 0
	ddi_dma_attr_t attr;
	zen_mpio_hotplug_t *hp = &fabric->zf_hotplug;
...
#if 0
	if (genoa_is_ruby()) {
		if (port->zpp_hp_type == SMU_HP_ENTERPRISE_SSD) {
...
genoa_hotplug_port_init(zen_pcie_port_t *port, void *arg)
{
	 ...
	 */
	if ((port->zpp_flags & ZEN_PCIE_PORT_F_HOTPLUG) == 0 ||
	    port->zpp_hp_type == SMU_HP_PRESENCE_DETECT) {
...
{
//	zen_hotplug_t *hp = &fabric->zf_hotplug;
	...
	if (!zen_mpio_rpc_send_hotplug_table(iodie, hp->zh_pa)) {
		return (false);
	}
#endif
