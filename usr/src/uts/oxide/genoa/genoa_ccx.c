...
genoa_thread_feature_init(void)
{
	...
	if (genoa_ccx_set_undoc_fields &&
	    (chiprev_matches(chiprev, X86_CHIPREV_AMD_GENOA_ANY))) {
...
genoa_core_ls_init(void)
{
	...
	v = AMD_LS_CFG_U_ZEN4_SET_DIS_SPEC_WC_REQ(v, 1);
...
{
	/* Nothing to set in MSR_AMD_DE_CFG for Genoa */
}
...
genoa_core_undoc_init(void)
{
	...
	if (!uarchrev_at_least(uarchrev, X86_UARCHREV_AMD_ZEN4_B0)) {
		v = AMD_FP_CFG_SET_UNKNOWN_52(v, 1);
	}
	if (uarchrev_matches(uarchrev, X86_UARCHREV_AMD_ZEN4_A0) ||
	    uarchrev_matches(uarchrev, X86_UARCHREV_AMD_ZEN4_B0)) {
		v = AMD_FP_CFG_SET_UNKNOWN_43(v, 1);
	}
...
{
// XXX: fix retreiving weights from SMU first
#if 0
