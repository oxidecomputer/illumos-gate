...
#define	VIONA_INTR_SET(vip, q) \
	(vip)->vip_status[VIONA_INTR_WORD(q)] |= VIONA_INTR_BIT(q)
#define	VIONA_INTR_TEST(vip, q) \
	(((vip)->vip_status[VIONA_INTR_WORD(q)] & VIONA_INTR_BIT(q)) != 0)
typedef struct vioc_intr_poll {
	uint16_t	vip_nrings;
	uint32_t	vip_status[VIONA_INTR_WORDS];
} vioc_intr_poll_t;
