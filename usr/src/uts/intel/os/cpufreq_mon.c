...
typedef struct cpufreq_mon_cpu {
	...
	hrtime_t	cfm_time;	/* window start */
 ...
 *
 * This is safe to call in any context: time is read with gethrtime_waitfree()
 * rather than gethrtime(), whose spin on hres_lock would deadlock if a sample
 * ran from an interrupt that had preempted the lock holder (tsc_tick() and
 * friends, at high level, on this CPU). Even so, the interrupt dispatchers
 * only call this when both the arriving interrupt and the interrupted context
 * are at or below LOCK_LEVEL. A sample is not worth adding latency to
 * high-level interrupt dispatch, nor taking on top of preempted high-level
 * code.
 */
void
cpufreq_mon_sample(void)
{
	 ...
	 * all we do is read the clock and compare. All time reads here use
	 * gethrtime_waitfree(). gethrtime() and its relatives spin until
	 * hres_lock is observed free, and that lock is held at high level by
	 * the likes of tsc_tick(). A sample taken from an interrupt that has
	 * preempted the lock holder would spin forever, as the holder cannot
	 * run again until the interrupt returns.
	 */
	now = gethrtime_waitfree();
	if (cfm->cfm_primed &&
	    (now - cfm->cfm_time) < cpufreq_mon_interval) {
		return;
	}
	...
	/*
	 * Publish. snapshot_hrtime is the time of this sample, so a consumer
	 * can compare it against the kstat snapshot time.
	 */
	...
	if (cfk != NULL) {
		...
		cfk->ck_interval_ns.value.ui64 = (uint64_t)dt;
		cfk->ck_snapshot_hrtime.value.ui64 = (uint64_t)now;
		if (have_eff)
