 ...
 * endian issue. In particular, when dealing with things related to the
 ...
 * we have called out all 16 MiB regions of the flash chip that may exist. This
 * handling isn't great and we probably should do something better in the
 * general case.
 */
...
t6_mfg_err2str(t6_mfg_t *t6mfg, t6_mfg_err_t err)
{
	...
	case T6_MFG_ERR_SOURCE_NOT_SUP:
		return ("T6_MFG_ERR_SOURCE_NOT_SUP");
	case T6_MFG_ERR_OUTPUT_NOT_SET:
		return ("T6_MFG_ERR_OUTPUT_NOT_SET");
	...
	case T6_MFG_ERR_FILE_TOO_SHORT:
		return ("T6_MFG_ERR_FILE_TOO_SHORT");
	case T6_MFG_ERR_INTERNAL:
		return ("T6_MFG_ERR_INTERNAL");
	case T6_MFG_ERR_LIBDEVINFO:
		return ("T6_MFG_ERR_LIBDEVINFO");
	case T6_MFG_ERR_OPEN_DEV:
		return ("T6_MFG_ERR_OPEN_DEV");
	case T6_MFG_ERR_LIBISPI:
		return ("T6_MFG_ERR_LIBISPI");
	case T6_MFG_ERR_UNKNOWN_DEV:
		return ("T6_MFG_ERR_UNKNOWN_DEV");
	case T6_MFG_ERR_INVALID_VPD:
		return ("T6_MFG_ERR_INVALID_VPD");
	case T6_MFG_ERR_BAD_FLAGS:
    ...
    t6_mfg_srom_region_f func, void *arg)
{
	...
	for (uint_t i = 0; i < ARRAY_SIZE(t6_srom_regions); i++) {
		...
		if (!t6_mfg_read(t6mfg, fd, t6mfg->t6m_data_buf,
	...
	for (cur = 0; cur < len; cur++) {
		...
		if (isalnum_l(str[cur], t6mfg->t6m_c_loc) == 0 &&
			...
			return (t6_mfg_error(t6mfg, T6_MFG_ERR_INVALID_VPD, 0,
			    ...
			    "ASCII alphanumeric character or a hyphen", cur,
			    str[cur]));
		}
    ...
    t6_mfg_validate_data_t *data)
{
	...
	size_t oqaque_off, verify_len;
	void *dataptr;

	if (reg->reg_type == T6_SROM_R_VPD_EXT) {
		oqaque_off = sizeof (vpd_ext);
		t6_mfg_srom_fill_vpd_ext(t6mfg, &vpd_ext);
		dataptr = &vpd_ext;
	} else {
		oqaque_off = sizeof (vpd);
		t6_mfg_srom_fill_vpd(t6mfg, &vpd);
		dataptr = &vpd;
	}
	verify_len = reg->reg_len - oqaque_off;

	if (memcmp(t6mfg->t6m_data_buf, dataptr, oqaque_off) != 0) {
		/*
	...
	if (memcmp(t6mfg->t6m_base_buf + oqaque_off,
	    t6mfg->t6m_data_buf + oqaque_off, verify_len) != 0) {
		uint32_t off;

		/*
		 * If we already hit an opaque error, don't bother updating the
		...
		data->tval_flags |= T6_VALIDATE_F_ERR_OPAQUE;
		for (off = oqaque_off; off < reg->reg_len; off++) {
    ...
    t6_mfg_srom_validate_f func, void *arg)
{
	...
	for (uint_t i = 0; i < ARRAY_SIZE(t6_srom_regions); i++) {
		...
		if (!t6_mfg_read(t6mfg, t6mfg->t6m_srom_base_fd,
		    t6mfg->t6m_base_buf, reg->reg_offset, reg->reg_len)) {
			return (B_FALSE);
		}
...
{
	if (nbytes > sizeof (t6mfg->t6m_data_buf)) {
		const char *msg = "internal error: asked to write beyond "
		    "buffer";
		upanic(msg, strlen(msg));
	}
