...
#define	RES_SET_NO_HOSTS_FALLBACK	"__sys_res_set_no_hosts_fallback"
extern void	__sys_res_set_no_hosts_fallback(void);

#define	RES_UNSET_NO_HOSTS_FALLBACK	"__sys_res_unset_no_hosts_fallback"
extern void	__sys_res_unset_no_hosts_fallback(void);
...
#define		__H_ERRNO			"__sys_h_errno"
extern int	*__sys_h_errno(void);

#define		RES_OVERRIDE_RETRY		"__sys_res_override_retry"
extern int	__sys_res_override_retry(int);
...
int	*(*get_h_errno)(void) = __sys_h_errno;
int	(*override_retry)(int) = __sys_res_override_retry;
...
/* Because we link against libresolv_sys.so.2, this is relatively easy. */
void
_nss_dns_init(void)
{
