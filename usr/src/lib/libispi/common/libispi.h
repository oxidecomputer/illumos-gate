 ...
 * API to change over time. This library is currently focused on SPI storage
 * devices, however there is no reason that it cannot be extended to other
 * classes of devices.
 *
 ...
 * size of such erases are, what the opcodes of those are, etc. all vary based
 * upon the actual spi device.
 *
 ...
 * bank into the actual command. This makes it hard to actually use as without a
 * bank, you are not guaranteed that the seeming bank 0 device is actually a
 * bank 0 device.
 *
 ...
 * that currently defaults to 500 ms. All programmed times are in miliseconds.
 * The granularity (ms) was chosen in part because we will likely sleep between
 * these as opposed to spin.
 */
typedef enum {
...
/*
 * Set the size of the flash in bytes. This may currently be set manually. In
 * the future, automatic discovery based on standard paramters would be useful.
 * The byte size must be a multiple of 64 KiB sectors when set.
 */
