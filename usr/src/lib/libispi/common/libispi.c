 ...
 */
#define	ISPI_DEF_TO_CHIP_ERASE	(5 * 1000 * 60)	/* 5min in ms */
#define	ISPI_DEF_TO_PROGRAM	(500)		/* 500ms */

/*
 * Various sentinal values in the internals of the library.
 */
#define	ISPI_CHIP_SIZE_UNKNOWN	UINT64_MAX
...
ispi_set_dev(ispi_t *ispi, int fd)
{
	...
	if (ispi->ispi_fd != -1) {
		return (ispi_error(ispi, ISPI_ERR_DEVICE_EXISTS, 0,
		    "SPI device already exists, fd %d", ispi->ispi_fd));
	}
...
{
	if (ispi->ispi_chip_size == ISPI_CHIP_SIZE_UNKNOWN) {
...
ispi_set_size(ispi_t *ispi, uint64_t size)
{
	...
	if (size > SPI_MAX_LEN_3B) {
		return (ispi_error(ispi, ISPI_ERR_SIZE_BEYOND_DEV_ADDR, 0,
		    "device size 0x%" PRIx64 "is beyond 3-byte addressable "
...
ispi_check_io(ispi_t *ispi, uint64_t offset, uint64_t len)
{
	...
	if (offset >= ispi->ispi_chip_size || len > ispi->ispi_chip_size ||
		...
		return (ispi_error(ispi, ISPI_ERR_IO_BAD_OFFSET, 0,
		    "combination of offset (0x%" PRIx64 ") and len (%" PRIu64
		    ") exceeds the chip size (0x%" PRIx64, offset, len,
		    ispi->ispi_chip_size));
	}
...
ispi_io_length(uint64_t offset, uint64_t len)
{
	 ...
	 * region. Because writes can't span this boundary, we have to further
	 * constrain this.
	 */
 ...
 * in the header file. In the future this should select opcodes automatically
...
ispi_chip_erase(ispi_t *ispi)
{
	...
	spidev_transfer_t erase_xfers[2] = { {
		.tx_buf = &wren_cmd,
		.len = sizeof (wren_cmd),
		.delay_usec = 0,
		.deassert_cs = 1,
...
ispi_init(void)
{
	...
	ispi->ispi_chip_size = ISPI_CHIP_SIZE_UNKNOWN;
