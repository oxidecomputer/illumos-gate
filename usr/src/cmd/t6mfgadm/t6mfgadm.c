...
t6mfgadm_progress_cb(const t6_mfg_progress_t *prog, void *arg)
{
	...
	case T6_MFG_PROG_ERROR:
		 ...
		 * thing we printed, but only if we've ever printed something.
		 */
	...
	case T6_MFG_PROG_IO_END:
		(void) printf("%sI/O: %8u/%u bytes (%4.1f%%)%s", head,
		    prog->tmp_offset, prog->tmp_total, (float)prog->tmp_offset /
...
t6mfgadm_list_ofmt_cb(ofmt_arg_t *ofarg, char *buf, uint_t buflen)
{
	...
	case T6MFGADM_LIST_FLASH:
		if (snprintf(buf, buflen, "/devices%s:spidev",
		    info->tmdi_path) >= buflen) {
			return (B_FALSE);
		}
		break;
	default:
...
t6mfgadm_list(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, ":Ho:p")) != -1) {
		...
		case 'p':
			...
			break;
		case ':':
			t6mfgadm_list_help("option -%c requires an argument",
			    optopt);
			exit(EXIT_USAGE);
		case '?':
	...
	argv += optind;
	if (argc != 0) {
		errx(EXIT_USAGE, "unknown extraneous arguments: %s", argv[0]);
	}
    ...
    t6mfgadm_info_t *info)
{
	...
	while ((c = getopt(argc, argv, ":d:o:P")) != -1) {
		...
		case ':':
			...
			exit(EXIT_USAGE);
		case '?':
			t6mfgadm_dev_read_help(type, "unknown option -%c",
			    optopt);
			exit(EXIT_USAGE);
		}
	}
