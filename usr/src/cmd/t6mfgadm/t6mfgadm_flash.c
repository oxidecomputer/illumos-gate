...
{
	(void) fprintf(f, "\tflash read [-P] -d device -o output\n");
}
...
t6mfgadm_flash_verify(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, ":b:d:f:F:i")) != -1) {
		...
		case ':':
			...
			exit(EXIT_USAGE);
		case '?':
			t6mfgadm_flash_verify_help("unknown option -%c",
			    optopt);
			exit(EXIT_USAGE);
		}
	}
...
t6mfgadm_flash_write_help(const char *fmt, ...)
{
	...
	(void) fprintf(stderr, "Usage:  %s flash write F fwfile "
	    "-d device | -f file [-P]\n", t6mfgadm_progname);
...
t6mfgadm_flash_version_help(const char *fmt, ...)
{
	...
	(void) fprintf(stderr, "Usage:  %s flash versions -f file | -d "
	    "device\n", t6mfgadm_progname);

	(void) fprintf(stderr, "\nShow T6 NOR flash firmware version "
	    "information.\n"
...
t6mfgadm_flash_version_ofmt_cb(ofmt_arg_t *ofarg, char *buf, uint_t buflen)
{
	...
	case T6MFGADM_FLASH_SHOW_VALID:
		if (strlcpy(buf, show->tfva_valid ? "yes" : "no", buflen) >=
		    buflen) {
			return (B_FALSE);
		}
		break;
...
static ofmt_field_t t6mfgadm_flash_version_ofmt[] = {
	{ "SECTION", 16, T6MFGADM_FLASH_SHOW_SEC,
	    t6mfgadm_flash_version_ofmt_cb },
	{ "VALID", 8, T6MFGADM_FLASH_SHOW_VALID,
	    t6mfgadm_flash_version_ofmt_cb },
	{ "VERSION", 20, T6MFGADM_FLASH_SHOW_VERS,
	    t6mfgadm_flash_version_ofmt_cb },
	{ NULL, 0, 0, NULL }
};
...
{
	if (argc == 0) {
		t6mfgadm_usage(t6mfgadm_cmds_flash, "missing required flash "
		    "subcommand");
		exit(EXIT_USAGE);
	}
